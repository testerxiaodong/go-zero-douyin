...
func TestAddTagLogic_AddTag(t *testing.T) {
	...
	addTagLogic := video.NewAddTagLogic(context.Background(), serviceContext)
	...
	videoRpcError := errors.New("VideoRpc.AddTag error")
	mockVideoRpc.EXPECT().AddTag(gomock.Any(), gomock.Any()).Return(nil, videoRpcError)

	// 参数校验成功，且videoRpc调用成功的mock
	mockValidator.EXPECT().Validate(gomock.Any()).Return("")
	mockVideoRpc.EXPECT().AddTag(gomock.Any(), gomock.Any()).Return(&pb.AddTagResp{}, nil)

	param := &types.AddTagReq{Name: utils.NewRandomString(10)}
	...
	testCases := []struct {
		...
		{
			name: "add_tag_with_validate_error",
			req:  param,
			err:  xerr.NewErrMsg(validatorError),
		},
		{
			name: "add_tag_with_video_rpc_error",
			req:  param,
