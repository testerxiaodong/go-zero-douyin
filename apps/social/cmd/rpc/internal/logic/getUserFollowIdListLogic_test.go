...
func TestGetUserFollowIdListLogic_GetUserFollowIdList(t *testing.T) {
	...
	mockFollowDo.EXPECT().GetUserFollowIdList(gomock.Any(), gomock.Any()).Return([]int64{}, dbError)

	// redis中没有数据，查询数据库成功的mock
	mockRedis.EXPECT().Exists(gomock.Any(), gomock.Any()).Return(false, nil)
	mockFollowDo.EXPECT().GetUserFollowIdList(gomock.Any(), gomock.Any()).Return([]int64{1, 2, 3}, nil)
	...
	testCases := []struct {
		...
		},
		{
			...
			req:  &pb.GetUserFollowIdListReq{UserId: 10},
			err:  nil,
		},
		{
			name: "get_user_follow_id_list_with_database_error",
			req:  &pb.GetUserFollowIdListReq{UserId: 10},
			err:  errors.Wrapf(xerr.NewErrCode(xerr.RPC_SEARCH_ERR), "get user follow id list from mysql failed, err: %v", dbError),
		},
		{
